...
function parseGraphQLError(error: unknown): ApiError {
  ...
  if (error instanceof TypeError && error.message.includes('fetch')) {
    return new ApiError('Unable to connect to the server. Please check your internet connection.', {
      code: 'NETWORK_ERROR',
      isNetworkError: true,
    });
  }
  ...
  if (error instanceof ClientError) {
    ...
    if (error.response?.status) {
      ...
      if (status >= 500) {
        return new ApiError('The server encountered an error. Please try again later.', {
          code: 'SERVER_ERROR',
        });
      }
    }
